...
class NpContentImporter {
    ...
    private $_importedSiteProductsIds = array();
    ...
    private function _importParameters($onlyHeaderFooter = false, $remove_previous_content = false) {
        ...
        if (!$onlyHeaderFooter && isset($data['productsJson']) && !empty($data['productsJson'])) {
            ...
            if ($new_products_data) {
                $this->importProductsEntities($new_products_data);
            }
    ...
    /**
     * Import site products to cms as custom post type like post/page
     *
     * @param array $new_products_data
     */
    public function importProductsEntities($new_products_data) {
        if (!isset($new_products_data['products'])) {
            return;
        }
        $prevIds = get_option('imported_site_products_ids');
        if ($prevIds) {
            $prevPostsEntities = get_posts(array('post_type' => 'site_product', 'numberposts' => -1));
            if (!$prevPostsEntities) {
                $prevIds = array(); // if not products entities remove old json data
                delete_option('imported_site_products_ids');
            }
            $this->_importedSiteProductsIds = $prevIds;
        }
        $data = &$new_products_data['products'];
        foreach ($data as $product_data) {
            $product_id = wp_insert_post(
                array(
                    'post_title' => isset($product_data['title']) ? $product_data['title'] : 'Product',
            ...
            if ($product) {
                ...
                $sku = isset($product_data['sku']) ? (string) $product_data['sku'] : '';
                if ($sku) {
                    add_post_meta($product_id, 'sku', $sku);
                }
                // set regular price
                $price = isset($product_data['fullPrice']) ? (string) $product_data['fullPrice'] : '';
                if ($price) {
                    add_post_meta($product_id, 'price', $price);
                }
                $product_images = _arr($product_data, 'images', '');
                if (count($product_images) > 0) {
                    add_post_meta($product_id, 'image', $product_images[0]);
                    add_post_meta($product_id, 'gallery_images', $product_images);
                }
            }
            $this->_importedSiteProductsIds[$product_data['id']] = $product_id;
        }
        update_option('imported_site_products_ids', $this->_importedSiteProductsIds);
    }
    ...
    public function import($remove_previous_content = false) {
        ...
        if ($remove_previous_content) {
            ...
            if (is_array($prev_data)) {
                ...
                foreach ($prev_data['site_products'] as $product_id) {
                    wp_delete_post($product_id);
                }
        ...
        update_option(
            ...
            array(
                ...
                'site_products' => $this->_importedSiteProductsIds,
