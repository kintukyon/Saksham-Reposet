...
class NpDataProvider {
    ...
    /**
     * Save products like post/page entities in cms
     *
     * @param $new_products_data
     */
    public function saveProductsEntities($new_products_data) {
        if (!isset($new_products_data['products'])) {
            return;
        }
        $prevProductEntities = get_posts(array('post_type' => 'site_product', 'numberposts' => -1));
        foreach ($prevProductEntities as $product) {
            if (isset($product->ID) && $product->ID) {
                wp_delete_post($product->ID); // remove prev data
            }
        }
        ...
        foreach ($data as $product_data) {
            ...
            $newIds[$product_data['id']] = $product_id;
        }
        update_option('imported_site_products_ids', $newIds);
    }
